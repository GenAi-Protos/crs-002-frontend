...
export function LookupPanel({ initial }: { initial?: string } = {}) {
  ...
  return <div className="space-y-5 py-6">
    <form onSubmit={(event) => { event.preventDefault(); if (input.trim()) void run(); }}>
      ...
      <div className="flex items-center gap-3 border border-rule bg-surface p-2 pl-3"><IconSearch className="shrink-0 text-mute" /><input id="observable" value={input} onChange={(event) => setInput(event.target.value)} className="h-8 min-w-0 flex-1 bg-transparent font-mono text-sm focus:outline-none" placeholder="IP address, domain, MD5, SHA-1 or SHA-256" required /><Button type="submit" variant="primary" disabled={pending || !input.trim()}>{pending ? "Checking…" : "Check"}</Button></div>
      <p className="mt-2 flex items-center gap-2 text-2xs text-mute"><IconEgress />Checks the accessible repository and configured reputation providers. External provider calls may leave the UAE region and are recorded.</p>
    </form>
    {error && <p role="alert" className="border border-rule bg-status-warn-fill p-3 text-sm text-status-warn-ink">{error}</p>}
    {pending && <SkeletonRows rows={4} />}
    {result && <>
      <div className="flex flex-wrap items-center justify-between gap-4 border-y border-rule py-4"><div><IndicatorChip value={result.observable} /><p className="mt-2 text-xs text-mute">{result.coverage.repositoryMatches} repository matches · {result.coverage.relatedCases} related investigations</p></div><AddToCase title={`Investigate ${defang(result.observable)}`} lookup={{ observable: result.observable, results: result.reputation.results, mode: result.reputation.mode, matches: result.matches, coverage: result.coverage }} /></div>
      ...
      <section><h3 className="mb-3 text-md font-semibold">Reputation providers</h3><div className="grid gap-3 sm:grid-cols-3">{result.reputation.results.map((provider) => <div key={provider.provider} className="border border-rule p-4"><div className="flex items-center justify-between gap-2"><h4 className="text-sm font-medium">{provider.provider}</h4><StatusPill tone={provider.status === "live" ? "good" : provider.status === "failed" ? "critical" : "warn"} label={provider.status.replaceAll("-", " ")} /></div><p className="mt-3 text-sm">{provider.verdict ?? "No verdict returned"}{provider.score !== null && provider.score !== undefined ? ` · Score ${provider.score}` : ""}</p>{provider.categories?.length > 0 && <p className="mt-2 text-xs text-mute">{provider.categories.join(", ")}</p>}{provider.error && <p className="mt-2 text-xs text-status-warn-ink">{provider.error}</p>}{provider.checkedAt && <p className="mt-2 text-2xs text-mute">{gstDateTime(provider.checkedAt)}</p>}</div>)}</div>{!result.reputation.results.length && <p className="text-sm">0 provider responses. Repository coverage is shown below.</p>}</section>
      <section><h3 className="mb-3 text-md font-semibold">Repository evidence</h3>{!result.matches.length && <p className="text-sm">0 accessible matches. A missing record does not establish that an observable is benign.</p>}<div className="divide-y divide-rule">{result.matches.map((match) => <article key={match.id} className="py-4"><div className="flex items-start justify-between gap-3"><h4 className="text-sm font-medium">{match.title}</h4><TlpBadge tlp={match.tlp} /></div><p className="mt-2 whitespace-pre-wrap text-sm">{defang(match.excerpt)}</p><p className="mt-2 text-2xs text-mute">{match.source} · {match.id}{match.observedAt ? ` · ${gstDateTime(match.observedAt)}` : ""}</p></article>)}</div></section>
      <section><h3 className="mb-3 text-md font-semibold">Related investigations</h3><ul className="space-y-2">{result.cases.map((item) => <li key={item.id} className="flex items-center gap-3 text-sm"><Link href={`/investigations/${encodeURIComponent(item.id)}`} className="text-link underline">{item.title}</Link><span className="text-xs text-mute">{item.status.replaceAll("-", " ")}</span></li>)}</ul>{!result.cases.length && <p className="text-sm">0 related investigations.</p>}</section>
    </>}
  </div>;
}
