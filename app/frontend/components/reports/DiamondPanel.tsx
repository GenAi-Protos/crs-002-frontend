  ...
  return (
    <div className="mt-3">
      ...
      <p className="mt-2 text-2xs text-mute">
        <span className="font-medium text-ink">{written} of 4</span> vertices
        written,{" "}
        <span className="font-medium text-ink">{diamond.edges.length} of 6</span>{" "}
    ...
    <div className="relative">
      ...
      <div className="relative grid grid-cols-1 gap-2 sm:grid-cols-3 sm:grid-rows-3 sm:gap-3">
        {VERTICES.map((v) => (
          <div
            key={v.key}
            className={`border border-rule bg-surface p-2.5 ${CELL[v.key]}`}
          >
            ...
            {editable ? (
              <textarea
                ...
                placeholder={v.hint}
                rows={3}
                className="mt-1.5 w-full resize-y border border-rule bg-surface p-1.5 text-xs leading-relaxed focus:border-cpx-green focus:outline-none"
              />
            ) : (
              <p className="mt-1.5 whitespace-pre-wrap text-xs leading-relaxed">
                {diamond[v.key].trim() === "" ? (
                  <span className="text-mute">Not stated.</span>
                ) : (
  ...
  return (
    <div className="mt-3">
      <table className="w-full border-collapse text-xs">
        ...
        <tbody>
          {ordered.map((e) => (
            <tr key={pairKey(e.source, e.target)} className="border-b border-rule">
              ...
              {editable && (
                <td className="py-1.5 text-right">
                  <button
                    ...
                    className="px-1 text-xs text-mute hover:text-cpx-red"
                  >
                    Remove
                  </button>
                </td>
      ...
      {editable && (
        <div className="mt-2 flex flex-wrap items-center gap-2">
          <select
            ...
            className="h-8 border border-rule bg-surface px-1.5 text-xs focus:border-cpx-green focus:outline-none"
          >
            {PAIRS.map(([from, to], i) => (
          ...
          <input
            ...
            className="h-8 min-w-0 flex-1 border border-rule px-2 text-xs focus:border-cpx-green focus:outline-none"
          />
          <button
            disabled={label.trim() === ""}
            onClick={() => {
